...
1. [Prepare](#prepare-implan-sector-allocation): Allocate spending by Implan sector and validate using `implan::check` functions.
2. [Input](#input-write-to-excel): Write Excel tabs for Implan import using `implan::input()`
3. [Output](#output-read-from-implan): Pull all Implan output (csv) into a table using `implan::output_read_csv()`
