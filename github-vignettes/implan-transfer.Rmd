...
Allocating spending to Implan sectors requires 1 or more crosswalk tables (typically built in Excel). The implan package includes example data to demonstrate the allocation process (see `read_excel` from the [readxl package](https://readxl.tidyverse.org/) for production scripts):
