...
filepath <- system.file(
  "extdata", "template", "implan-output.xlsm", package = "implan", mustWork = TRUE
)
file.copy(filepath, "tmp.xlsm")
```

I've written R functions that use a slightly different approach. Rather than copy/pasting into Excel, you'll save output results into csv. There are typically 5 results sets of interest, and examples are included here:

```{r}
output <- system.file("extdata", "output", "hunt", package = "implan", mustWork = TRUE)
list.files(output)
```
