...
The non-R approach incorporates steps 1 & 2 in an Excel workbook. I included a basic example in this package (these become far more complex when scaling up):
...
Tom has an existing Excel macro-based approach for pulling together economic contributions. I've included an example here if you want to adapt it:
...
With the R approach, rather than copy/pasting into Excel, you'll save output results into csv, where each implan activity should have its own folder. Two example activities are included in the sample data:
...
Typically, there are five different sets of results that you'll be interested in:
...
The `output_read_csv()` function is included to pull all implan output csv files into an R list:
