...
Writing to Excel (for Implan import) can do done with a `input()` function. This function wraps two steps that I think are worth distinguishing:

### input_prep()

To prepare for implan import, we first convert spending by sector to the necessary Industry/Commodity format using `input_prep()` (which has separate versions for Industry and Commodity). Each destination Excel tab requires two tables (header & data), so `input_prep()` returns a list of two data frames.:
