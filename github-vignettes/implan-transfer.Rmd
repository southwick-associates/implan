...
2. [Write Excel tabs (Industry/Commercial) for Implan import](#write-to-excel)
3. [Pull all Implan output (csv) into one table of results](#read-from-csv)
...
Allocating spending results to implan sectors requires 1 or more crosswalk tables (typically built in Excel, see the [readxl package](https://readxl.tidyverse.org/)). The implan package includes example data to demonstrate the allocation process:
...
For the sample data, spending must first be reallocated from the "item" level to the "implan category" level using the `share` variable. Spending is specified by 2 dimensions (`type`, `item`); for which share must sum to 100%:
