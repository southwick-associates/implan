...
data(spending) # spending by activity_group-type-item
spending
```

### Item to Category

Spending must first be reallocated from the item level (e.g., from survey data) to the category level (e.g., "Food - Groceries", etc.). Spending in the sample data is specified by 2 dimensions (`type`, `item`) for which `share` in the crosswalk table must sum to 100%:
...
In the same way, allocating to sectors uses a crosswalk at the `category` dimension (also referred to as implan sectoring schemes):
