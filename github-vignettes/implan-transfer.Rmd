...
The implan package includes example data for demonstration. The starting point is a table of total spending estimates by activity-type-item. The `activity_group` variable is included for joining with the item-to-category crosswalk table described below.
...
Writing to Excel (for Implan import) can do done with the `input()` function. This function wraps two steps that I think are worth distinguishing:

### input_prep()

To prepare for implan import, we first convert spending by sector to the necessary Industry/Commodity format using `input_prep()` (which has separate convenience functions for Industry and Commodity). Each destination Excel tab requires two tables (header & data), so `input_prep()` returns a list of two data frames.:
...
After prepartion, the data can be written to Excel with `input_write()`. Note that when you are ready for Implan import, you'll first need to open the `.xlsx` file and save to the legacy `.xls` format which Implan requires.
...
In practice it's useful to wrap the prep/write steps into a single function:
...
The `input()` function also allows grouping at an arbitrary number of dimensions, which are written to separate Excel sheets. For example, we could slice the results by activity and type (equip vs trip):
...
for (i in acts) {
    ...
    input_prep_ind(df, paste0(i, "Ind")) %>% input_write("tmp2.xlsx")
    input_prep_comm(df, paste0(i, "Comm")) %>% input_write("tmp2.xlsx")
}
openxlsx::getSheetNames("tmp2.xlsx")
```
...
write_dims <- function(spend_sector, outfile, dimension = "act") {
    ...
    for (i in dims) {
        ...
        input_prep_ind(df, paste0(i, "Ind")) %>% input_write(outfile)
        input_prep_comm(df, paste0(i, "Comm")) %>% input_write(outfile)
    }
}
